...
| Hero-Headline **zwei Sätze** (2026-06-10): H1 „Growing digital commerce. Closing the gap between strategy and results." · Subline „E-Commerce & Marketing Outcomes through MarTech, Data & AI." | Solo-„Closing the Implementation Gap" las sich wie Aufgabenbeschreibung (Berater-/Transformations-Lesart), nicht wie Führungsmandat. Ein VP führt das Geschäft; das Gap-Schließen ist der Fokus *darin*. „Growing" (statt „Running") passt zur Evidenz (auf €1,6 Mrd. skaliert) und zur Outcome-Sprache der Pillars. „Gap between strategy and results" statt Fachbegriff reduziert den Registerbruch. **„Closing the Implementation Gap" bleibt Dachthese / roter Faden** in Texten, Posts, Interview-Story, ist aber nicht mehr alleinige H1. Zahlen (€1,6 Mrd., Märkte) bewusst in der Hero-Bio, nicht in der Subline | ✅ |
...
## 17. Bereinigung 2026-09-07: Seitenbestand vor Internationalisierung

- Direkter Auftrag von Karsten: vor der geplanten DE/EN-Struktur den bestehenden Seitenbestand bereinigen, damit nur kanonische, tatsächlich genutzte Seiten in die spätere Internationalisierung übernommen werden. Kein Inhalt neu formuliert, keine neue Informationsarchitektur, keine neuen Seiten.
- Nav zeigt seit heute mit „Über mich“ direkt auf `/track-record` (kein separater Track-Record-Link mehr); die frühere `/ueber-mich`-Seite (eigenes Narrativ, IA-Entscheidung aus Abschnitt 11) ist dadurch inhaltlich redundant und ohne interne Verlinkung.
- `/ueber-mich` wird nach demselben Muster wie `/track-record/deep-dive` (Abschnitt 16) zu einem reinen `Astro.redirect("/track-record", 301)`; die alte Route bleibt für Altlinks/Bookmarks erreichbar, ohne Dopplung im Index.
- `/ueber-mich` aus der Sitemap-Allowlist in `astro.config.mjs` entfernt (analog zur bereits ausgeschlossenen `/track-record/deep-dive`).
- Geprüft und unverändert gelassen, da kanonisch bzw. aktiv verlinkt: `/commercial-management`, `/data-ai-digital-product`, `/digital-product-customer-experience` (verlinkt aus `Commercial.astro`), `/leadership`, `/track-record`, alle `/commercial-growth/[slug]`, `/agentic-ai/*`, `/e-commerce-marketing/*`, `/martech/*`, Legal-Seiten. Keine „alten Commercial-Detailseiten“ als Repo-Leichen gefunden — die zuvor separate Data/AI-Decisioning-Fläche wurde bereits in `data-ai-digital-product.astro` konsolidiert (Commit `bc223fe`).
- Zur Kenntnis, nicht verändert: `/schwerpunkte` und `/agentic-ai/awareness-war-nie-der-engpass` sind aktueller, kanonkonformer Inhalt, aber ohne interne Verlinkung aus Nav/Homepage (nur über Sitemap/Direktlink erreichbar). Kein Löschkandidat — beide passen inhaltlich zum aktuellen Rahmen, das ist ein Verlinkungs-, kein Bestandsproblem. Separate Entscheidung nötig, falls verlinkt werden soll.
- `CLAUDE.md` (Struktur-Abschnitt) auf den bereinigten Seitenbestand aktualisiert (Verweis auf `ueber-mich` als eigene Seite entfernt, Redirect-Hinweis ergänzt).
- Geändert: `src/pages/ueber-mich.astro` (Inhalt entfernt, Redirect), `astro.config.mjs`, `CLAUDE.md`, dieses Protokoll. Build lokal verifiziert.
